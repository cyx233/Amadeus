...
import GenerateProgressModal from './modals/GenerateProgressModal';
...
export default function TaskBoardToolbar({
  ...
  const { availableTags, selectedTag, selectTag, currentProject, refreshTasks } = useTaskMaster();
  const [isPrdDropdownOpen, setIsPrdDropdownOpen] = useState(false);
  const [deletingPrd, setDeletingPrd] = useState<string | null>(null);
  const { progress: generateProgress, isRunning: isGenerating, start: startGenerate, stop: stopGenerate, dismiss: dismissGenerate } = useGenerateTasks();
  // The tag currently generating (for per-row spinner state in the dropdown).
  const generatingTag = generateProgress?.status === 'running' ? generateProgress.tag : null;
  ...
  const handleGenerateTasks = (prd: PrdFile) => {
    if (!currentProject?.projectId || isGenerating) return;
    setIsPrdDropdownOpen(false);
    startGenerate({
      projectId: currentProject.projectId,
      fileName: prd.name,
      tag: prdNameToTag(prd.name),
      onComplete: (tag) => {
        selectTag(tag); // jump the board to the freshly-generated set
        void refreshTasks();
      },
    });
  };
  ...
  const handleDeletePrd = async (prd: PrdFile) => {
    ...
    try {
      ...
      // If the deleted set was selected, let the server pick a default on refresh.
      if (selectedTag === slug) {
        selectTag('master');
      }
      await refreshTasks();
  ...
  return (
    <>
                    ...
                    {isPrdDropdownOpen && (
                      <div className="absolute right-0 top-full z-30 mt-2 w-56 rounded-lg border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800">
                        <div className="p-2">
                          ...
                          {/* Single-select: one PRD's task set at a time. Clicking a
                              row selects it (radio semantics) and closes the menu. The
                              selected row is highlighted; the ✨/📄/🗑 actions stay. */}
                            ...
                            return (
                              ...
                              >
                                <button
                                  ...
                                  disabled={!hasTasks}
                                  title={!hasTasks ? t('tags.noTasksYet', 'Generate tasks first') : undefined}
                                  className={cn(
                          ...
                          {/* Orphan task sets whose PRD file is gone (incl. a non-empty
                              master). Still selectable so their tasks stay reachable. */}
                          {orphanTags.length > 0 && (
                            <>
                              <div className="my-1 border-t border-gray-200 dark:border-gray-700" />
                              {orphanTags.map((tagName) => (
                                <div
                                  ...
                                  className={cn(
                                    'group flex items-center gap-2 rounded px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-700',
                                    selectedTag === tagName && 'bg-purple-50 dark:bg-purple-900/20'
                                  )}
                                >
                                  <button
      ...
      {generateProgress && (
        <GenerateProgressModal progress={generateProgress} onStop={stopGenerate} onClose={dismissGenerate} />
      )}
    </>
  );
}
